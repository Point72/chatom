...
class CombinedE2ETest:
        ...
        try:
            ...
            try:
                async with asyncio.timeout(60.0):
                    async for message in self.symphony_backend.stream_messages(channel_id=self.symphony_stream_id):
    ...
    async def test_slack_to_symphony(self):
        ...
        try:
            ...
            try:
                async with asyncio.timeout(60.0):
                    async for message in self.slack_backend.stream_messages(channel_id=self.slack_channel_id):
                        # First message from a user (skip_own=True by default)
                        received_message = message
                        break
